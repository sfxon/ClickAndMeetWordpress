class mvCmTimer {
		...
		showEditor(self) {
				//Show dialog box
				jQuery('#AppointmentEditor').modal({
						keyboard: false,
						backdrop: 'static'
				});
				...
				jQuery('#editor-time').clockTimePicker({
		...
		resetCustomerFieldValues(self) {
				...
				jQuery('#editor-custom-form-dropdown').val(self.custom_form_dropdown);
				...
				jQuery('#editor-last-save-datetime').val(self.last_save_datetime);
		}
		...
		initDeleteButtonAction(self) {
				jQuery('#mv-editor-delete').off('click');
				jQuery('#mv-editor-delete').on('click', function() {
						self.hideAllErrors();
		...
		checkInputFieldsForDeleting(self) {
				var error = false;
				var appointment_id = jQuery('#editor-appointment-id').val();
		...
		buildPostDataArrayForDeleting(self) {
				...
				var post_data = {
						appointment_id: parseInt(jQuery('#editor-appointment-id').val()),
						day: self.day,
						month: self.month,
						year: self.year	
				};
		...
		fetchAllDataFromInputFields(self) {
				...
				self.custom_form_dropdown = jQuery('#editor-custom-form-dropdown').val();
				...
				self.last_save_datetime = jQuery('#last_save_datetime').val();
				...
				self.last_save_datetime = jQuery('#editor-last-save-datetime').val();
		}
